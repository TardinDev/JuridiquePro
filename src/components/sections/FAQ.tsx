...
export function FAQ() {
  ...
  return (
    <section ref={sectionRef} className="section-padding bg-[#CCD0D8]">
          ...
          <Accordion type="single" collapsible className="space-y-3">
            {FAQ_ITEMS.map((item, i) => (
              <AccordionItem
                key={i}
                value={`item-${i}`}
                className="faq-item overflow-hidden rounded-xl border border-border bg-[#D4D7DE] px-6 card-glow data-[state=open]:border-royal/20 data-[state=open]:shadow-sm"
              >
